...
$(document).ready(function(){
	...
	sliderNav.find('.item > img').each(function( index ) {
	...
	sliderNav.owlCarousel({
		...
		var $currentItem = $('.owl-item', sliderNav).eq(event.item.index);
	...
	/* Slider show dots */
	var sliderDots = $("#slider_dots");
	
	sliderDots.find('.item > img').each(function( index ) {
		var $img = $(this).attr('src');
		$(this).parent().addClass('have-bg');
		$(this).parent().css('background-image', 'url('+$img+')');
	});
	sliderDots.owlCarousel({
		items: 1,
		autoplay: false,
		...
		var $currentItem = $('.owl-item', sliderDots).eq(event.item.index);
		var $elemsToanim = $currentItem.find("[data-animation-in]");
	...
	$(".currency-drop .dropdown-menu a").on("click", function(){
    	var val = $(this).attr('data-attr');
        $(".currency-drop button").html(val);
	})
	$(".flag-drop .dropdown-menu a").on("click", function(){
    	var val = $(this).attr('data-attr');
        $(".flag-drop button img").attr("src",val);
    })
});
